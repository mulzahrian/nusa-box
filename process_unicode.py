...
# Step 1: cleanup_unicode.py logic
print("=" * 60)
...
try:
    ...
    with open(path, 'w', encoding='utf-8') as f:
        f.write(text)
    print(f'Done. Total removed/replaced: {total_step1}')
    ...
    for i, l in enumerate(lines[1449:1510], start=1449):
        for j, c in enumerate(l):
            if ord(c) > 127 or (ord(c) < 32 and c not in '\t'):
                msg = f'L{i+1}C{j+1}: U+{ord(c):04X} {repr(c)}'
                print(msg)
                non_ascii_found.append(msg)
    ...
    print(f'Total stripped: {total_step3}')
